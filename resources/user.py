...
blp = Blueprint("Users", "uses", description="Operations on tags")
...
class TagsInStore(MethodView):
    ...
    def post(self, tag_data, store_id):
        if UserModel.query.filter(UserModel.store_id == store_id).first():
            abort(400, message="A tag with that name already exists in that store.")

        tag = UserModel(**tag_data, store_id=store_id)
...
class LinkTagsToItem(MethodView):
    ...
    def post(self, item_id, tag_id):
        ...
        tag = UserModel.query.get_or_404(tag_id)
